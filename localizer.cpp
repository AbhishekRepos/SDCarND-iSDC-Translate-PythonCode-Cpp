...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
	...
	int totalSize = row*column;
	float value = 1.0/totalSize;
	...
	for (auto row_index = 0; row_index < row; ++row_index) {
		for (auto column_index = 0; column_index < column; ++column_index) {
			newGrid[row_index][column_index] = value;
		}
	}
  ...
  float blurring) 
{
  ...
  // your code here
	auto row = beliefs.size();
	auto column = beliefs[0].size();
	auto new_row = 0, new_column = 0;

    newGrid = zeros(row, column);
	for(auto row_index = 0; row_index < row; ++row_index) {
        for(auto column_index = 0; column_index < column; ++column_index) {
           new_row = (row_index + dy + row) % row;
           new_column = (column_index + dx + column) % column;
           newGrid[new_row][new_column] = beliefs[row_index][column_index];
        }
    }
	...
	float p_miss) 
{
	...
	for (auto row_index = 0; row_index < row; ++row_index){
		for (auto column_index = 0; column_index < column; ++column_index){
			newGrid[row_index][column_index] = beliefs[row_index][column_index];
			if(grid[row_index][column_index] == color) {
				newGrid[row_index][column_index] *= p_hit;
			}
			else {
				newGrid[row_index][column_index] *= p_miss;
			}
        }
    }
