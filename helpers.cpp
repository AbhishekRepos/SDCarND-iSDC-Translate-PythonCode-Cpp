...
vector< vector<float> > normalize(vector< vector <float> > grid) {
	...
	// todo - your code here
  	float total = 0.0;
	auto row = grid.size(), coloumn = grid[0].size();

	for (auto row_index = 0; row_index < row; row_index++)	{
		for (auto column_index = 0; column_index < coloumn; column_index++)	{
			total += grid[row_index][column_index];
		}
	}

	newGrid = zeros(row, coloumn);

	for (auto row_index = 0; row_index < row; row_index++)	{
		for (auto column_index = 0; column_index < coloumn; column_index++)	{
			newGrid[row_index][column_index] = grid[row_index][column_index] / total;
		}
	}
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
    ...
    vector <vector <float>> temp_grid = { {corner, adjacent, corner},
    ...
    newGrid = zeros(row, column);

	for (auto row_index = 0; row_index < row; ++row_index)	{
		for (auto column_index = 0; column_index < column; ++column_index) 	{
			float grid_val = grid[row_index][column_index];
			for (auto delta_row_index = -1; delta_row_index < 2; ++delta_row_index) {
				for (auto delta_column_index = -1; delta_column_index < 2; ++delta_column_index) {
					auto new_row_index = (row_index + delta_column_index + row) % row;
					auto new_column_index = (column_index + delta_row_index + column) % column;
					float multiplier = temp_grid[delta_row_index + 1][delta_column_index + 1];
					newGrid[new_row_index][new_column_index] += grid_val * multiplier;
				}
			}
		}
	}
